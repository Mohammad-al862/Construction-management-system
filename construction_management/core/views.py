...
def my_profile(request):
    ...
    if manager:
        ...
        return Response({**user_data, 'manager': manager_data}, status=status.HTTP_200_OK)
    elif supervisor:
        supervisor_data = SupervisorSerializer(supervisor).data
        return Response({**user_data, 'supervisor': supervisor_data}, status=status.HTTP_200_OK)
